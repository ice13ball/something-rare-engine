...
| **WAPHA** (tailings dams base) | `tailings` | **CC0** (Dryad), cite | Hudson-Edwards et al. 2023, doi:10.5061/dryad.j3tx95xmg; companion Macklin et al. 2023 (*Science* 381:1345). **Not Maus et al.** ⛔ **Global Tailings Portal (GRID-Arendal) fields withdrawn 2026-09-23** — tailing.grida.no/about asks for permission to download the TSF dataset, which we never obtained. Every Portal-derived column (hazard rating, ownership, dam specifications, safety-review answers, disclosure link/notes) is withheld from every serving surface, on every row, and the 361 Portal-only facilities (`data_source='grid'`) are not served at all. Rows and columns remain in the database — nothing was deleted. Single source of truth: `backend/domains/land/common.py` `TAILINGS_SERVED_WHERE` / `TAILINGS_PORTAL_COLUMNS`. Restore if/when GRID-Arendal grants written permission. |
