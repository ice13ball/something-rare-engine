...
export function GeotracesHexPanel({ properties }: { properties: Record<string, unknown> }) {
  const { t } = useTranslation("legend");
  const selected = useMapStore((s) => s.geotracesDecadeFilters);
  ...
  return (
    <div className="space-y-2 text-sm">
      ...
      <p className="text-white/60 text-xs">
        ...
        individual station to open its trace-metal depth profile.
      </p>
    </div>
  );
}
